...
const CommunityPage = () => {
  ...
  // ✅ NEW: Check if user has already joined this community on page load
  useEffect(() => {
    const checkJoinStatus = async () => {
      try {
        ...
        const res = await axios.get(
          `http://localhost:5000/api/communities/${id}/joined`,
          { headers: { Authorization: token } }
        );
        setIsJoined(res.data.joined);
      } catch (err) {
        console.error("Failed to check join status");
      }
    };
    checkJoinStatus();
  }, [id]);
  ...
  // ✅ NEW: Join / Leave community handler
  const handleJoinToggle = async () => {
    ...
    try {
      ...
      setIsJoined(res.data.joined);
      toast({
        title: res.data.joined ? "Joined! 🎉" : "Left community",
        description: res.data.joined
          ? `You're now part of ${community.name}`
      ...
      toast({
        variant: "destructive",
        title: "Failed",
        description: "Could not update membership. Try again.",
      });
    } finally {
        ...
        <div className="container mx-auto px-4">
          ...
          >
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
              ...
              {/* ✅ UPDATED: Dynamic Join/Leave Button */}
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button
                  ...
                  className={
                    isJoined
                      ? "border-green-500 text-green-500 hover:bg-red-500/10 hover:text-red-400 hover:border-red-400 transition-all duration-300"
                      : "btn-glow"
                  }
                >
                  {joinLoading ? (
